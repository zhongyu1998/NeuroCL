...
def gen_pt(cnf_dir, trace_dir, pt_dir, n_cpu=1):
    ...
    for file_name in sorted(os.listdir(cnf_dir)):
        ...
        if os.path.isfile(file_path):
            if ("cnf" in file_name or "dimacs" in file_name) and \
                not os.path.isfile(pt_dir + "/" + file_name + ".pt"):
...
def gen_pt_single(arg_list):
    ...
    backbone_name = cnf_name + ".backbone"
    ...
    if not os.path.isfile(backbone_file_path):
        ...
        backbone_file_path = None

    trace_name = cnf_name + ".lrat"
    trace_file_path = trace_dir + "/" + trace_name

    data = cnf_to_bipartite(cnf_file_path, backbone_file_path, trace_file_path)
...
def cnf_to_bipartite(cnf_file_path, backbone_file_path, trace_file_path):
    ...
    if training_flag:
        ...
        try:
            ...
            if exit_status == 10:
                print(f"SATISFIABLE! LRAT file successfully generated for {cnf_name}.")
            elif exit_status == 20:
                print(f"UNSATISFIABLE! LRAT file successfully generated for {cnf_name}.")
            elif exit_status == 0:
                print(f"Warning: Stop after {max_conflicts} conflicts for {cnf_name}.")
        except Exception as e:
...
if __name__ == '__main__':
    dataset, level, stage = 'k-clique', 'hard', 'test'
    mode = f"{dataset}_{level}_{stage}"
    ...
    if stage == "train":
        ...
        print("Error: Stage can only be train, valid, or test.")
        sys.exit(1)
